    ...
    def handle(self, *args, **options):
        ...
        for agent in Agent.objects.all():
            ...
            if not prompt_file_path.exists():
                logger.warning(f'Prompt file for agent {agent.name} not found. Creating default: {prompt_file_path}')
        ...
        try:
            ...
            logger.exception(f"An unexpected error occurred in the agent application loop.")
    ...
    def _process_agent_cycle(self, agent):
        ...
        if perceptions_to_process.exists():
            ...
            for perception in perceptions_to_process:
                logger.info(f"Processing perception {perception.id} for agent '{agent.name}'.")
                ...
                if command_match:
                    ...
                    logger.debug(f"Found command '{command_content}' in perception {perception.id}.")

                    # Add to CommandQueue
                    CommandQueue.objects.create(agent=agent, command=command_content.replace('|', ' ', 1))
                    logger.info(f"Queued command '{command_content.replace('|', ' ', 1)}' for agent '{agent.name}'.")

                    # Mark perception as sent
                    perception.text = re.sub(command_pattern, f"[command|{command_content}]sent", perception.text)
                    logger.debug(f"Marked command '{command_content}' as 'sent' in perception {perception.id}.")
                    
                elif memory_load_match:
                    memory_key = memory_load_match.group(1)
                    logger.debug(f"Found memory load command '{memory_key}' in perception {perception.id}.")

                    # Add to CommandQueue as a memory-load command
                    CommandQueue.objects.create(agent=agent, command=f"memory-load {memory_key}")
                    logger.info(f"Queued memory-load command 'memory-load {memory_key}' for agent '{agent.name}'.")
                    
                    

                    # Mark perception as sent
                    perception.text = re.sub(memory_load_pattern, f"[memory|load|{memory_key}]sent", perception.text)
                    logger.debug(f"Marked memory load command '{memory_key}' as 'sent' in perception {perception.id}.")
...
""".join(processed_perception_texts)
            else:
                agent.perception = """
""".join(processed_perception_texts)
            ...
            agent.save()
            logger.debug(f"Agent '{agent.name}' perception updated and last_retrieved timestamp set.")
        ...
        if llm_entry:
            logger.info(f"Processing completed LLM response {llm_entry.id} for agent '{agent.name}'.")
            ...
            while True:
                llm_command_match = re.search(llm_command_pattern, modified_llm_response)
                llm_memory_load_match = re.search(llm_memory_load_pattern, modified_llm_response)
                llm_memory_create_match = re.search(r"\[memory\|create\|(.*?)\]", modified_llm_response)
                ...
                if llm_command_match:
                    ...
                    logger.debug(f"Found command '{content}' in LLM response {llm_entry.id}.")
                elif llm_memory_load_match:
                    match = llm_memory_load_match
                    pattern_type = "memory|load"
                    content = match.group(1)
                    command_to_queue = f"memory-load {content}"
                    replacement_text = f"processed_memory_load_{content}"
                    logger.debug(f"Found memory load command '{content}' in LLM response {llm_entry.id}.")
                elif llm_memory_create_match:
                    match = llm_memory_create_match
                    pattern_type = "memory|create"
                    content = match.group(1)
                    command_to_queue = f"memory-create {content.replace('|', ' ', 1)}"
                    replacement_text = f"processed_memory_create_{content.replace('|', '_')}"
                    logger.debug(f"Found memory create command '{content}' in LLM response {llm_entry.id}.")
                else:
                    break # No more patterns found

                # Add to CommandQueue
                CommandQueue.objects.create(agent=agent, command=command_to_queue)
                logger.info(f"Queued command '{command_to_queue}' from LLM response for agent '{agent.name}'.")

                # Replace the found pattern
                start_index = match.start()
                end_index = match.end()
                modified_llm_response = modified_llm_response[:start_index] + replacement_text + modified_llm_response[end_index:]
                logger.debug(f"Marked command as 'sent' in LLM response.")

            # Append modified LLM response to agent's perception field
            if agent.perception:
                agent.perception += """
""" + modified_llm_response # Gemini do not touch this please
            else:
                agent.perception = modified_llm_response
            agent.perception = agent.perception[-5000:] # Keep only the last 5000 characters
            agent.save()
            ...
            llm_entry.save()
            logger.debug(f"LLM request {llm_entry.id} marked as 'delivered' and response updated.")

            agent.phase = 'acting' # Agent has just processed an LLM response, so it's acting
            agent.save()
        ...
        if active_llm_requests:
            ...
            if agent.phase != 'thinking':
                logger.debug(f"Agent '{agent.name}' has active LLM requests, setting phase to 'thinking'.")
        ...
        for mem_id in agent.memoriesLoaded:
            try:
                ...
                logger.warning(self.style.WARNING(f"Warning: Loaded memory ID {mem_id} not found for agent '{agent.name}'."))
        # Gemini do not touch this block of code please
        if loaded_memories_values:
            llm_prompt_parts.append("## Loaded Memories:\n" + "\n".join(loaded_memories_values))
        if agent.perception:
            llm_prompt_parts.append("## Agent Perception History:\n" + agent.perception)
        ...
        if 'waiting' in agent.flags and agent.flags['waiting']:
            ...
            if timezone.now() < wait_until:
                logger.info(f"Agent {agent.name} is waiting until {wait_until.strftime('%Y-%m-%d %H:%M:%S')}.")
